...
import { GetArtifactController } from '@/presentation/controller'

const makeSut = () => {
  const getArtifactStub: GetArtifact = {
    get: async () => {
      return { found: [], notFound: [] } as GetArtifactResults
    },
    getFull: async () => {
...
const validRequest: GetArtifactController.Request = { keys: [{ userid: 'any_id', dtAdded: 'any_date' }] }

describe('Get Artifact Controller', () => {
  test('Should call Validation with correct data', async () => {
    const { sut, validationStub } = makeSut()
    const validateSpy = jest.spyOn(validationStub, 'validate')
    const httpRequest: GetArtifactController.Request = validRequest
    await sut.handle(httpRequest)
    if (httpRequest.keys != null) { expect(validateSpy).toHaveBeenCalledWith(httpRequest.keys[0]) }
  })
  ...
  test('Should call GetArtifact with correct data', async () => {
    ...
    const httpRequest: GetArtifactController.Request = validRequest
    await sut.handle(httpRequest)
    if (httpRequest.keys != null) { expect(getArtifactSpy).toHaveBeenCalledWith(httpRequest) }
  })
  ...
  test('Should return 400 if id is invalid', async () => {
    ...
    const httpRequest: GetArtifactController.Request = { keys: [{ userid: 'any_id', dtAdded: 'invalid_date' }] }
